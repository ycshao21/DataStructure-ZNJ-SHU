...
namespace myds {
template <class T, T defaultValue = T()> struct SquareMatrix {
public:
  SquareMatrix(int dim = 0) : m_data(dim, std::vector<T>(dim, defaultValue)) {}
  SquareMatrix(const SquareMatrix<T, defaultValue> &) = default;
  SquareMatrix<T, defaultValue> &
  operator=(const SquareMatrix<T, defaultValue> &) = default;
