...
namespace algorithm {
template <class DistTy, DistTy infinity> struct PathNode {
...
template <class VertTy, class WeightTy, WeightTy infinity>
void PrintPath(const std::vector<VertTy> &vertices,
               const std::vector<PathNode<WeightTy, infinity>> &paths,
...
void dijkstra(const myds::AdjMatWDirGraph<VertTy, WeightTy, infinity> &g,
  ...
  std::priority_queue<VertNode, std::vector<VertNode>, std::greater<VertNode>>
      minHeap;
  ...
  while (!minHeap.empty()) {
    ...
    for (int j = 0; j < vertNum; j++) {
      if (j == minVertIndex ||
          g.m_AdjMat[minVertIndex][j] ==
              infinity) // Skip itself and unreachable vertices.
        continue;
  ...
  for (int i = 0; i < vertNum - 1; i++) {
    ...
    for (int fromIndex = 0; fromIndex < vertNum; fromIndex++) {
      ...
      for (int toIndex = 0; toIndex < vertNum; toIndex++) {
        if (g.m_AdjMat[fromIndex][toIndex] ==
            infinity) // Skip unreachable vertices.
          continue;

        WeightTy tempDist =
            paths[fromIndex].dist + g.m_AdjMat[fromIndex][toIndex];
...
void FloydWarshall(const myds::AdjMatWDirGraph<VertTy, WeightTy, infinity> &g,
  ...
  // Initialize paths.
  using PathNode = PathNode<WeightTy, infinity>;
  std::vector<std::vector<PathNode>> paths(vertNum,
                                           std::vector<PathNode>(vertNum));
