...
namespace Algorithm {
template <class WeightTy> struct MST_Edge {
  int fromIndex{};
  int toIndex{};
  WeightTy weight{};
...
template <class VertTy, class WeightTy, WeightTy infinity>
WeightTy Kruskal(const myds::AdjListWUndirGraph<VertTy, WeightTy, infinity> &g,
  ...
  std::priority_queue<MST_Edge, std::vector<MST_Edge>, std::greater<MST_Edge>>
      minHeap;
...
template <class VertTy, class WeightTy, WeightTy infinity>
WeightTy Prim(const myds::AdjListWUndirGraph<VertTy, WeightTy, infinity> &g,
              std::ostream *out = &std::cout) {
  int vertNum = g.GetNumOfVertices();
  if (vertNum == 1) {
    if (out)
  ...
  while (!minHeap.empty()) {
    ...
    ss << "(" << ++cnt << ") "
       << "(" << g.m_Vertices[curEdge.fromIndex] << ", "
       << g.m_Vertices[curEdge.toIndex] << "): " << curEdge.weight << std::endl;
    totalWeight += curEdge.weight;
...
template <class VertTy, class WeightTy, WeightTy infinity>
WeightTy
ReverseDelete(const myds::AdjListWUndirGraph<VertTy, WeightTy, infinity> &g,
              std::ostream *out = &std::cout) {
  int vertNum = g.GetNumOfVertices();
  if (vertNum == 1) {
    if (out)
  ...
  while (mst.GetNumOfEdges() > vertNum - 1) {
    ...
    // Remove the edge:
    auto fromIter = std::ranges::find_if(mst.m_AdjList[curEdge.fromIndex],
                                         Graph::FindEdgeFn(curEdge.toIndex));
    mst.m_AdjList[curEdge.fromIndex].erase(fromIter);
    auto toIter = std::ranges::find_if(mst.m_AdjList[curEdge.toIndex],
                                       Graph::FindEdgeFn(curEdge.fromIndex));
    mst.m_AdjList[curEdge.toIndex].erase(toIter);
    mst.m_EdgeNum--;

    // Add the edge back if it breaks the connectivity.
    if (!mst.IsConnected()) {
      mst.m_AdjList[curEdge.fromIndex].push_front(
          myds::AdjListEdge{curEdge.toIndex, curEdge.weight});
      mst.m_AdjList[curEdge.toIndex].push_front(
          myds::AdjListEdge{curEdge.fromIndex, curEdge.weight});
      mst.m_EdgeNum++;
    }
  }
    ...
    for (const auto &e : mst.m_AdjList[fromIndex]) {
      if (fromIndex < e.toIndex) {
        ss << "(" << ++cnt << ") "
           ...
           << mst.m_Vertices[e.toIndex] << "): " << e.weight << std::endl;
...
template <class VertTy, class WeightTy, WeightTy infinity>
bool TreeFindPath(
    const myds::AdjListWUndirGraph<VertTy, WeightTy, infinity> &tree,
    int prevIndex, int fromIndex, int endIndex,
    std::stack<MST_Edge<WeightTy>> &path) {
...
bool IsUnique(const myds::AdjListWUndirGraph<VertTy, WeightTy, infinity> &g) {
  ...
  for (const auto &e : discardedEdges) {
    TreeFindPath(mst, std::numeric_limits<int>::max(), e.fromIndex, e.toIndex,
                 path);
    while (!path.empty()) {
