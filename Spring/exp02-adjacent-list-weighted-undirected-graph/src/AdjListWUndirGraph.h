...
public:
  ...
  void SetVertex(const VertTy &oldVert, const VertTy &newVert) {
    m_Vertices[GetIndexOfVertex(oldVert)] = newVert;
  }
...
public:
  ...
  friend WeightTy
  Algorithm::Kruskal(const AdjListWUndirGraph<VertTy, WeightTy, infinity> &g,
                     std::ostream *out);
  template <class VertTy, class WeightTy, WeightTy infinity>
  friend WeightTy
  Algorithm::Prim(const AdjListWUndirGraph<VertTy, WeightTy, infinity> &g,
                  std::ostream *out);
  template <class VertTy, class WeightTy, WeightTy infinity>
  friend WeightTy Algorithm::ReverseDelete(
      const AdjListWUndirGraph<VertTy, WeightTy, infinity> &g,
      std::ostream *out);

public:
  // @brief Depth-first search from one vertex.
  void DFS(const VertTy &from,
           void (*visit)(const VertTy &) = OutputVertex) const;
...
private:
  ...
  void _DFS(int sourceIndex, std::vector<uint8_t> &visited,
            void (*visit)(const VertTy &) = OutputVertex) const;
  void _BFS(int sourceIndex, std::vector<uint8_t> &visited,
            void (*visit)(const VertTy &) = OutputVertex) const;

  static void OutputVertex(const VertTy &vert) { std::cout << vert << " "; }

  template <class VertTy, class WeightTy, WeightTy infinity>
  friend bool Algorithm::TreeFindPath(
      const AdjListWUndirGraph<VertTy, WeightTy, infinity> &tree, int prevIndex,
      int fromIndex, int endIndex, std::stack<MST_Edge<WeightTy>> &path);

  template <class VertTy, class WeightTy, WeightTy infinity>
  friend bool
  Algorithm::IsUnique(const AdjListWUndirGraph<VertTy, WeightTy, infinity> &g);

private:
  class FindEdgeFn // For finding edge according to the toIndex.
  {
  public:
    FindEdgeFn(int toIndex) : m_ToIndex(toIndex) {}
    bool operator()(const Edge &edge) const {
      return edge.toIndex == m_ToIndex;
    }
...
template <class VertTy, class WeightTy, WeightTy infinity>
AdjListWUndirGraph<VertTy, WeightTy, infinity>::AdjListWUndirGraph(
...
template <class VertTy, class WeightTy, WeightTy infinity>
int AdjListWUndirGraph<VertTy, WeightTy, infinity>::GetNumOfConnectedComponent()
    const {
  if (m_Vertices.empty())
    throw NoVertex();
...
template <class VertTy, class WeightTy, WeightTy infinity>
int AdjListWUndirGraph<VertTy, WeightTy, infinity>::GetDegree(
    const VertTy &vert) const {
  int vertIndex = GetIndexOfVertex(vert);
  return (int)m_AdjList[vertIndex].size();
}

template <class VertTy, class WeightTy, WeightTy infinity>
const VertTy *AdjListWUndirGraph<VertTy, WeightTy, infinity>::GetFirstAdjVertex(
...
template <class VertTy, class WeightTy, WeightTy infinity>
VertTy *AdjListWUndirGraph<VertTy, WeightTy, infinity>::GetFirstAdjVertex(
    const VertTy &from) {
...
template <class VertTy, class WeightTy, WeightTy infinity>
const VertTy *AdjListWUndirGraph<VertTy, WeightTy, infinity>::GetNextAdjVertex(
  ...
  auto edgeIter =
      std::ranges::find_if(m_AdjList[fromIndex], FindEdge(afterIndex));
...
template <class VertTy, class WeightTy, WeightTy infinity>
VertTy *AdjListWUndirGraph<VertTy, WeightTy, infinity>::GetNextAdjVertex(
...
template <class VertTy, class WeightTy, WeightTy infinity>
WeightTy AdjListWUndirGraph<VertTy, WeightTy, infinity>::GetWeight(
    const VertTy &from, const VertTy &to) const {
  int fromIndex = GetIndexOfVertex(from), toIndex = GetIndexOfVertex(to);
...
template <class VertTy, class WeightTy, WeightTy infinity>
int AdjListWUndirGraph<VertTy, WeightTy, infinity>::GetIndexOfVertex(
    const VertTy &vert) const {
  int i = 0;
...
template <class VertTy, class WeightTy, WeightTy infinity>
void AdjListWUndirGraph<VertTy, WeightTy, infinity>::InsertVertex(
    const VertTy &vert) {
  if (std::ranges::find(m_Vertices, vert) != m_Vertices.end())
...
template <class VertTy, class WeightTy, WeightTy infinity>
void AdjListWUndirGraph<VertTy, WeightTy, infinity>::EraseVertex(
    const VertTy &vert) {
  int vertIndex = GetIndexOfVertex(vert);
  ...
  for (int fromIndex = 0; fromIndex < m_AdjList.size(); fromIndex++) {
    if (fromIndex == vertIndex)
      continue;
    auto edgeIter =
        std::ranges::find_if(m_AdjList[fromIndex], FindEdgeFn(vertIndex));
    if (edgeIter != m_AdjList[fromIndex].end())
  ...
  if (vertIndex != lastIndex) {
    ...
    for (int fromIndex = 0; fromIndex < lastIndex; fromIndex++) {
      if (fromIndex == vertIndex)
        continue;
      auto edgeIter =
          std::ranges::find_if(m_AdjList[fromIndex], FindEdgeFn(lastIndex));
      if (edgeIter != m_AdjList[fromIndex].end())
...
template <class VertTy, class WeightTy, WeightTy infinity>
void AdjListWUndirGraph<VertTy, WeightTy, infinity>::SetWeight(
...
template <class VertTy, class WeightTy, WeightTy infinity>
void AdjListWUndirGraph<VertTy, WeightTy, infinity>::InsertEdge(
  ...
  if (std::ranges::find_if(m_AdjList[fromIndex], FindEdgeFn(toIndex)) !=
      m_AdjList[fromIndex].end())
...
template <class VertTy, class WeightTy, WeightTy infinity>
void AdjListWUndirGraph<VertTy, WeightTy, infinity>::EraseEdge(
  ...
  // Erase (from, to).
  auto fromIter =
      std::ranges::find_if(m_AdjList[fromIndex], FindEdgeFn(toIndex));
  if (fromIter == m_AdjList[fromIndex].end())
    throw EdgeNotFound();
...
template <class VertTy, class WeightTy, WeightTy infinity>
void AdjListWUndirGraph<VertTy, WeightTy, infinity>::_DFS(
    int sourceIndex, std::vector<uint8_t> &visited,
    void (*visit)(const VertTy &)) const {
...
template <class VertTy, class WeightTy, WeightTy infinity>
void AdjListWUndirGraph<VertTy, WeightTy, infinity>::_BFS(
    int sourceIndex, std::vector<uint8_t> &visited,
    void (*visit)(const VertTy &)) const {
...
template <class VertTy, class WeightTy, WeightTy infinity>
void AdjListWUndirGraph<VertTy, WeightTy, infinity>::DFS(
    const VertTy &source, void (*visit)(const VertTy &)) const {
...
template <class VertTy, class WeightTy, WeightTy infinity>
void AdjListWUndirGraph<VertTy, WeightTy, infinity>::BFS(
    const VertTy &source, void (*visit)(const VertTy &)) const {
  int fromIndex = GetIndexOfVertex(source);
  std::vector<uint8_t> visited(m_Vertices.size(), 0);
...
template <class VertTy, class WeightTy, WeightTy infinity>
void AdjListWUndirGraph<VertTy, WeightTy, infinity>::PrintAdjacentList(
...
template <class VertTy, class WeightTy, WeightTy infinity>
void AdjListWUndirGraph<VertTy, WeightTy, infinity>::PrintEdges(
    std::ostream &out) const {
  int edgeCnt = 0;
  for (int fromIndex = 0; fromIndex < m_AdjList.size(); fromIndex++) {
    for (const auto &e : m_AdjList[fromIndex]) {
