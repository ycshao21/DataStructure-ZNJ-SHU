...
请使用**Visual Studio**并在**C++20以上的标准**运行项目。  
我没有直接使用具有浓厚SJ风格的参考代码，而是参考了Sartaj Sahni的*Data Structures, Algorithms, and Aplications in C++*，重写了自己的版本。一方面是因为一般人驾驭不了参考代码的独特设计，另一方面是想锻炼自己的coding能力。  
关于课程安排，一个学期共有一次个人实验和四次小组实验。个人实验当场验收，完成可以作为加分项，这里不提供代码；小组实验需要一起完成代码和报告，三个人一组，验收会按照组号顺序或倒序进行，所以每个小组会分到其中一个实验，**组号可以自己选**。  
最后感谢一起合作过的三位组员@donghaidiwang @ZhiXinYo @1nv1nce. 
...
现要求完成第4种，即移动当前节点左子树到其后继左子树的方式，并用多组测试数据对教材实现的和本题实现的这2种算法进行性能测试，分析比较它们的查找性能。
